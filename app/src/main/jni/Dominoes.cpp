...
Java_com_qualcomm_QCARSamples_Dominoes_Dominoes_onQCARInitializedNative(JNIEnv* env, jobject)
{
    ...
    jmethod_playSound = NULL;
    jmethod_showDeleteButton = NULL;
    jmethod_hideDeleteButton = NULL;
    jmethod_enableStartButton = NULL;
    jmethod_disableStartButton = NULL;
    jmethod_toggleStartButton = NULL;
    ...
    shouldClearSelectedDomino = false;
...
Java_com_qualcomm_QCARSamples_Dominoes_DominoesRenderer_initNativeCallback(JNIEnv* env, jobject obj)
{
    ...
    jmethod_playSound = jniEnv->GetMethodID(javaClass, "playSound", "(IF)V");
    jmethod_showDeleteButton = jniEnv->GetMethodID(javaClass, "showDeleteButton", "()V");
    jmethod_hideDeleteButton = jniEnv->GetMethodID(javaClass, "hideDeleteButton", "()V");
    jmethod_enableStartButton = jniEnv->GetMethodID(javaClass, "enableStartButton", "()V");
    jmethod_disableStartButton = jniEnv->GetMethodID(javaClass, "disableStartButton", "()V");
    jmethod_toggleStartButton = jniEnv->GetMethodID(javaClass, "toggleStartButton", "()V");
...
Java_com_qualcomm_QCARSamples_Dominoes_DominoesRenderer_renderFrame(JNIEnv* , jobject)
{
    ...
    if (state.getNumTrackableResults() > 0) {
        ...
        const QCAR::VirtualButtonResult* button = imageTargetResult->getVirtualButtonResult("startButton");
...
Java_com_qualcomm_QCARSamples_Dominoes_Dominoes_nativeTouchEvent(JNIEnv* , jobject, jint actionType, jint pointerId, jfloat x, jfloat y)
{
    ...
    // Ignore touch events while the simulation is running
    if (simulationRunning) {
        return;
    }
...
playSoundEffect(int soundIndex, float volume)
{
    ...
    // Call a Java method that plays a sound
    getJNIEnv()->CallVoidMethod(javaObj, jmethod_playSound, soundIndex, volume);
}
...
showDeleteButton()
{
    ...
    // Call a Java method that shows the delete button
    getJNIEnv()->CallVoidMethod(javaObj, jmethod_showDeleteButton);
}
...
{
    // For this application, buttons are handled by the Android SDK
    // Call a Java method that hides the delete button
    getJNIEnv()->CallVoidMethod(javaObj, jmethod_hideDeleteButton);
}
...
{
    // For this application, buttons are handled by the Android SDK
    // Call a Java method that enables the start button
    getJNIEnv()->CallVoidMethod(javaObj, jmethod_enableStartButton);
}
...
{
    // For this application, buttons are handled by the Android SDK
    // Call a Java method that disables the start button
    getJNIEnv()->CallVoidMethod(javaObj, jmethod_disableStartButton);
}
...
{
    // For this application, buttons are handled by the Android SDK
    // Call a Java method that toggles the start button
    getJNIEnv()->CallVoidMethod(javaObj, jmethod_toggleStartButton);
}
...
updateAugmentation(const QCAR::Trackable* trackable, float dt)
{
    ...
    // Respond to a clear selected domino request
    if (shouldClearSelectedDomino) {
        selectedDomino = NULL;
        shouldClearSelectedDomino = false;
    }
...
renderAugmentation(const QCAR::Trackable* trackable)
{
    ...
    const Texture* const blueGlowTexture = textures[2];
    ...
    // Disable depth testing to avoid z-fighting between domino glow polygons
    ...
    if (dominoCount > 0 && (selectedDomino == NULL || selectedDominoIndex != 0)) {
        ...
        SampleUtils::translatePoseMatrix(domino->position.data[0], domino->position.data[1], 0.0f, transformPtr);
    ...
    // Render a blue glow under the selected domino
    if (selectedDomino != NULL) {
        ...
        glBindTexture(GL_TEXTURE_2D, blueGlowTexture->mTextureID);
        renderCube(transformPtr);
    }
...
void VirtualButton_UpdateCallback::QCAR_onUpdate(QCAR::State& state)
{
    if (shouldUpdateButton)
    {
        ...
        // The trackable should be an image target
        // Note that virtual buttons only work with image targets
        assert(trackable);
...
updateDominoTransform(Domino* domino)
{
    ...
    SampleUtils::translatePoseMatrix(domino->position.data[0], domino->position.data[1], 0.0f, transformPtr);
    SampleUtils::rotatePoseMatrix(domino->pivotAngle, 0, 0, 1, transformPtr);
    SampleUtils::translatePoseMatrix(kDominoScaleX, 0.0f, 0.0f, transformPtr);
    SampleUtils::rotatePoseMatrix(domino->tiltAngle, 0, 1, 0, transformPtr);
    SampleUtils::translatePoseMatrix(-kDominoScaleX, 0.0f, kDominoScaleZ, transformPtr);
    SampleUtils::scalePoseMatrix(kDominoScaleX, kDominoScaleY, kDominoScaleZ, transformPtr);
}
...
updatePickingTransform(Domino* domino)
{
    ...
    SampleUtils::translatePoseMatrix(domino->position.data[0], domino->position.data[1], 0.0f, transformPtr);
    SampleUtils::rotatePoseMatrix(domino->pivotAngle, 0, 0, 1, transformPtr);
    SampleUtils::translatePoseMatrix(0.0f, 0.0f, kDominoScaleZ, transformPtr);
    SampleUtils::scalePoseMatrix(kDominoScaleX * 2, kDominoScaleY, kDominoScaleZ, transformPtr);
}
...
setSelectedDomino(Domino* domino)
{
    ...
    if (domino == NULL) {
        shouldClearSelectedDomino = true;
    }
    else {
